...
function plugin({ filePath, db }: { filePath: string; db: DB }) {
  return (tree: any) => {
    // addNode(tree, db, filePath);
    // console.log(db.nodes.find({ type: 'root' }));
    // console.log(db.nodes.find({ filePath }));
    console.log(JSON.stringify(tree));
  };
}
export default plugin;

function addNode(node: RawNode, db: DB, filePath: string): ElementNodeDoc {
  if (node.type === 'root' && filePath) {
    const file = db.getFile(filePath);
    if (file) {
      console.log({ file });
      deleteNode(file.$loki, db);
      db.unlinkFileToTag(filePath);
    }
    node.filePath = filePath;
  } else if (node.type === 'text') {
    return addTextNode(node, db, filePath);
  }

  if (node.children) {
    const children = node.children.map((child: RawNode) => {
