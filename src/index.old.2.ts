...
import { DB, newDB, getDb } from './db';
import { ID, Query } from './types';
import { isLeaf, Tree, Branch, Leaf, cyrb53, diff, getTree, toString } from './diff';
...
getDb().then((db: any) => {
  const processor2 = remark()
    ...
    .use(() => (tree: any) => {
      try {
        updateDB(tree, '/Users/vamshi/Dropbox/life/test.md', db);
      } catch (e) {
        console.error(e);
      }
    })
    ...
    .process(vfile.readSync('/Users/vamshi/Dropbox/life/test.md'), function (error, file) {
      ...
      // jsonfile.writeFileSync('q4.json', { data: db.getAll() }, { spaces: 2 });
    });
...
//TODO: delete all children and dependents
function addNode(node: any, db: any) {
  const n = db.addNode({ type: node.type, value: node.value });
  return n;
}
function updateNode(node: any, childIds: ID[], db: any) {
  return db.updateNode(
    Object.assign(
      {},
      { meta: node.meta, childIds, $loki: node.$loki, type: node.type, filePath: node.filePath, value: node.value },
    ),
  );
}

function addTree(node: any, db: DB) {
  const addedNode = addNode(node, db);
  if (node.children) {
    updateNode(
      addedNode,
      node.children.map((child: any) => addTree(child, db).$loki),
      db,
    );
  }
  return addedNode;
}
function deleteTree(nodeId: any, db: DB) {
  const node = db.getNode(nodeId);
  const childIds = node.childIds;
  childIds.forEach((childId: ID) => {
    deleteTree(childId, db);
  });
  db.deleteNode(nodeId);
}
export function treeDiff(oldItem: Tree | undefined, newItem: Tree, db: any, dbItem: any) {
  if (!newItem) {
    return;
  }
  if (!oldItem) {
    // addTree(newItem, db);
    if (isLeaf(newItem)) {
      addNode({ type: newItem.type, value: newItem.value }, db);
    } else {
      updateNode(
        dbItem,
        newItem.children.map((newItemChild) => addTree(newItemChild, db).$loki),
        db,
      );
      //   // newItem.children.forEach((child, index) => treeDiff(undefined, child, db, dbChildren[index]));
    }
  } else if (isLeaf(newItem) && isLeaf(oldItem)) {
    if (newItem.value !== oldItem.value) {
      db.updateNode(
        Object.assign(
          {},
          { $loki: dbItem.$loki, type: newItem.type, value: newItem.value, filePath: newItem.filePath },
        ),
      );
    }
  } else if (!isLeaf(oldItem) && !isLeaf(newItem)) {
    const hashListNew = newItem.children.map(toString);
    const hashListOld = (oldItem.children || []).map(toString);
    const { additions, deletions, updates } = diff(hashListNew, hashListOld);
    let dbChildren: any[] | undefined;
    if (additions.length > 0) {
      dbChildren = hashListNew.map((hash, index) =>
        additions.includes(index)
          ? addTree(newItem.children[index], db)
          : oldItem.children[hashListOld.findIndex((str: string) => str === hash)],
      );
      updateNode(
        dbItem,
        dbChildren.map((c) => c.$loki),
        db,
      );
    }
    if (updates.length > 0) {
      if (newItem.type === 'listItem') {
        //handle checkbox
      }
    }
    deletions.forEach((index: number) => deleteTree(oldItem.children[index].$loki, db));

    newItem.children.forEach((child: Tree, index: number) => {
      if (additions.includes(index)) {
        return; // dont need to compare when its additions
      }
      treeDiff((oldItem.children || [])[index], child, db, dbChildren ? dbChildren[index] : oldItem.children[index]);
    });
  }
}

export function updateDB(tree: any, filePath: string, db: any) {
  let rootTree = (db.getFile(filePath) as unknown) as Branch;

  //db.getAll().filter((n: any) => n.filePath === filePath && n.type === 'root')[0];

  const oldTree = rootTree?.$loki !== undefined ? getTree(rootTree.$loki, db) : undefined;
  if (!rootTree) rootTree = db.addNode({ type: 'root', filePath });
  treeDiff(oldTree, tree, db, rootTree);
}